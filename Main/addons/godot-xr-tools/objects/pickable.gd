...
#20240710T1556; allow coll to be detected, while held
@export var can_coll = false
...
func pick_up(by: Node3D) -> void:
	#print("pickable.gd: pick_up(): by is ",by)
	# Skip if not enabled
	if not enabled:
		return
	...
	if !can_coll: #20240710T1558: allow coll
		freeze = true
	...
	if by.picked_up_ranged:
		if ranged_grab_method == RangedMethod.LERP:
			...
			#20240710T1815; slow progress, also the items, aren't snapping to the "center"
				#p_grabber : Grabber,
		...
		#var grab := Grab.new(grabber, self, by_grab_point, true)
		var grab := Grab.new(grabber, self, by_grab_point, false) #20240710T1816;
		_grab_driver = XRToolsGrabDriver.create_snap(self, grab)
